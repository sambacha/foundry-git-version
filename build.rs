fn main() {
    ...
    // especially a fresh repository. This can cause CI builds to take a very
    // long time due to cargo having to relink the executable every time.

    // This problem is solved by having the build script test the existence of
    ...
    // git_output is None if the git executable is not available.
    let git_output = std::process::Command::new("git")
        .args([
            "describe",
            "--match=v*",
    ...
    if let Some(git_hash) = git_hash {
        println!("cargo:rustc-env=FOUNDRY_BUILD_GIT_HASH={git_hash}");
    }
}